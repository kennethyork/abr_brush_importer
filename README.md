...
Files that have not changed since the last import are skipped, so the startup
scan is fast.  The plugin keeps a small JSON database
(`abr_importer_cache/abr_import_db.json`) to track what has already been
imported.
...
- **Automatic import** — drop folder scanned on every Krita startup; optional
  continuous background watcher for hands-off importing without restarting
...
1. Create the `abr_brushes` folder listed in the table above.
2. Copy your `.abr` files into it.
3. Restart Krita — brushes are imported automatically in the background.

That's it.  The brushes will appear in the **Predefined Brush Tips** panel.
